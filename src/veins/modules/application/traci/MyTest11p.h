...
struct resource
{
    ...
    std::queue<task> pending_tasks; // 等待分配的任務(FIFO)
    std::list<task> handling_tasks; // 車輛正在處理的任務
    std::list<task> waiting_tasks; // 交給其他UAVorMEC處理，等待回傳的任務
...
struct UAV_MapData
{
    double generate_time; // UAV beacon生成的時間
    double Delay; // UAV beacon 傳送過來的delay
    double Delay_to_MEC; // UAV be傳送至附近MEC的delay，若UAV附近沒有MEC，則為-1
    int remain_cpu;
    int remain_memory;
};
