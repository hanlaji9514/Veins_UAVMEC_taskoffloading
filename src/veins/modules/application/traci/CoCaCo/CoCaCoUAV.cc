...
#include <veins/modules/application/traci/CoCaCo/CoCaCoUAV.h>
...
#include "veins/modules/application/traci/common.h" // 所有全域變數宣告在common.h中，並在GHDSFCar.cc中定義

using namespace veins;

Define_Module(veins::CoCaCoUAV);





void CoCaCoUAV::initialize(int stage)
{
...
CoCaCoUAV::CoCaCoUAV() : UAV_resource(200,200) {} // 每個UAV要有自己的UAV_resource

void CoCaCoUAV::onWSA(DemoServiceAdvertisment* wsa)
{
...
void CoCaCoUAV::onWSM(BaseFrame1609_4* frame)
{
    ...
    else if(std::string(wsm->getName()).substr(0, 14) == "UAV_MEC_handle") // 車輛傳送給UAV要求轉傳給MEC(1/3)，以及UAV自己算(1/3)的任務請求
    {
        ...
        double energy_UAVtoMEC = (parameter.E1_UAV + parameter.E3_MEC + (parameter.E2_UAV * Distance_to_MEC * Distance_to_MEC)) * received_t.packet_size;
    ...
    else if(std::string(wsm->getName()).substr(0, 15) == "MECTaskSendBack") // UAV收到MEC處理完成的任務，要轉傳回給車輛
    {
        ...
        for(auto it = UAV_resource.waiting_tasks.begin(); it != UAV_resource.waiting_tasks.end();)
        {
            ...
            if (it->full_packet_size == Back_FullPacketSize)
            {
                ...
                SendBacktoNode->setSenderPosition(curPosition);
...
void CoCaCoUAV::handleReceivedTask()
{
...
void CoCaCoUAV::computeFlyingEnergy()
{
...
void CoCaCoUAV::handlePositionUpdate(cObject* obj)
{
