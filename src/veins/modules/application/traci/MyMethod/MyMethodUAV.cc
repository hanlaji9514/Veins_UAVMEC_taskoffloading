...
#include <veins/modules/application/traci/MyMethod/MyMethodUAV.h>
...
MyMethodUAV::MyMethodUAV() : UAV_resource(200,200) {} // 每個UAV要有自己的UAV_resource

void MyMethodUAV::onWSA(DemoServiceAdvertisment* wsa)
{
...
void MyMethodUAV::onWSM(BaseFrame1609_4* frame)
{
    ...
    */
    else if(std::string(wsm->getName()).substr(0, 14) == "UAV_MEC_handle") // 車輛傳送給UAV要求轉傳給MEC
    {
        ...
        TraCIDemo11pMessage *SendtoMEC = new TraCIDemo11pMessage;
        populateWSM(SendtoMEC);
        SendtoMEC->setSenderPosition(curPosition);
        SendtoMEC->setByteLength(received_t.packet_size);
        SendtoMEC->setSenderAddress(myId);
        SendtoMEC->setRecipientAddress(Nearest_MEC);
        SendtoMEC->setName(s.c_str());
...
void MyMethodUAV::onBM(BeaconMessage* bsm)
{
...
void MyMethodUAV::handleSelfMsg(cMessage* msg)
{
...
void MyMethodUAV::handleReceivedTask()
{
...
void MyMethodUAV::computeFlyingEnergy()
{
...
void MyMethodUAV::handlePositionUpdate(cObject* obj)
{
